...
EARLY_STOPPING_CONFIG = {
    'min_epochs': 50,       # 减少最少训练轮数
    'check_epochs': 20,     # 检查最近多少个epoch
    'patience': 200,         # 连续多少个epoch没有改善就停止
...
# 改进的数据预处理函数
def preprocess_labels_improved(labels):
    """改进的标签预处理 - 保持原始坐标范围"""
    # 不进行归一化，保持原始坐标范围
    labels_processed = tf.cast(labels, tf.float32)
    
    # 添加轻微的数据增强
    if tf.random.uniform([]) < 0.2:  # 20%概率添加噪声
        noise = tf.random.normal(tf.shape(labels_processed), 0.0, 1.0)
...
def improved_accuracy(y_true, y_pred):
    ...
    # 使用更宽松的PCK阈值：图像尺寸的10%
    pck_threshold = 25.6  # 10% of 256 pixels = 25.6 pixels
...
model.compile(
    ...
    loss=improved_loss,
    metrics=[improved_accuracy]
)

print(f"\n开始训练...")
print(f"总轮数: {total_epoch}")
print(f"批处理大小: {batch_size}")
print(f"学习率: 0.0001 (带梯度裁剪)")
...
for epoch in range(total_epoch):
    ...
    for batch_idx, (images, labels) in enumerate(train_pbar):
        try:
            # 改进的数据预处理
            labels_processed = preprocess_labels_improved(labels)
            ...
            with tf.GradientTape() as tape:
                ...
                loss = improved_loss(labels_processed, predictions)
        ...
        # 计算准确率
        accuracy = improved_accuracy(labels_processed, predictions)
    ...
    # 保存记录
    with open("train_record_improved.json", "w") as f:
    ...
    if (epoch + 1) % 5 == 0:
        ...
        plt.tight_layout()
        plt.savefig("plots/training_curves_improved.png", dpi=150, bbox_inches='tight')
        plt.close()
...
print(f"最佳模型保存在: {checkpoint_dir}/best_model.weights.h5")
